      ...
      let newConsumptionCO2Emission = "";
            ...
            try {
                const res = await fetch(API, {
                    ...
                    body: JSON.stringify({
                        ...
                        electricity: parseInt(newConsumptionElectricity),
                        gas: parseInt(newConsumptionGas),
                        other: parseInt(newConsumptionOther),
                        total_consumption: parseInt(newConsumptionTotalConsumption),
                        co2_emission: parseInt(newConsumptionCO2Emission)
                    })
                });
                ...
                if (resultStatus === 201) { 
                    ...
                    console.error(`Error creating Consumption; Status received: ${resultStatus}`);
                }
            } catch (error) {
                console.error("Error in POST to annual_consumptions:", error);
            }
        }
        async function deleteAnnualConsumption(aacc, year) {
            ...
            try {
                ...
                console.error("Error in DELETE to annual_consumptions:", error);
            }
        }
